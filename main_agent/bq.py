...
class BQClient:
    def __init__(self):
        ...
        self.bq_client = bigquery.Client(
            location=BQ_LOCATION, project=self.project_id, credentials=self.creds
        )
        logging.info(
            "BQ client project=%s location=%s sa_email=%s",
            self.project_id,
            BQ_LOCATION,
            self.sa_email,
        )
...
if __name__ == "__main__":
    ...
    df = bq_client.execute_query(
        qu, "test_query"
    ).to_dataframe()  # add create_bqstorage_client=True later
    print(df)
