...
class RootAgent(BaseAgent):
    ...
    def run_pipeline(self, ctx: InvocationContext, session_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (intent_state.get("query_type") == "anomaly" or is_continuing_anomaly) and self.anomaly_agent is not None:
            logger.info("[Root] ✓ Routing to anomaly agent")
        ...
        # ✅ signature for cache (the key fix)
        question_sig = final_intent.get("question") or combined_q
        ...
        final_state = dict(explain_state)
